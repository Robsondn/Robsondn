...
Robô que executa a cada 5 minutos: consome a API do JMS via requests, processa dados de viagens e dispara alertas via Feishu Bot.  
`Python` `requests` `Feishu Bot API` `Pandas`

> Autenticação via token de sessão · Deduplicação inteligente de alertas · Roda 24h de forma autônoma
...
Dashboard de KPIs operacionais com robôs de coleta automática do BRDrive.  
